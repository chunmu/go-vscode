...
import { isAbsolute, resolve, join, dirname } from '../../base/common/path.ts';
import { IProcessEnvironment, isMacintosh, isWindows } from '../../base/common/platform.ts';
import { randomPort } from '../../base/common/ports.ts';
import { whenDeleted, writeFileSync } from '../../base/node/pfs.ts';
import { findFreePort } from '../../base/node/ports.ts';
import { watchFileContents } from '../../platform/files/node/watcher/nodejs/nodejsWatcherLib.ts';
import { NativeParsedArgs } from '../../platform/environment/common/argv.ts';
import { buildHelpMessage, buildStdinMessage, buildVersionMessage, NATIVE_CLI_COMMANDS, OPTIONS } from '../../platform/environment/node/argv.ts';
import { addArg, parseCLIProcessArgv } from '../../platform/environment/node/argvHelper.ts';
import { getStdinFilePath, hasStdinWithoutTty, readFromStdin, stdinDataListener } from '../../platform/environment/node/stdin.ts';
import { createWaitMarkerFileSync } from '../../platform/environment/node/wait.ts';
import product from '../../platform/product/common/product.ts';
import { CancellationTokenSource } from '../../base/common/cancellation.ts';
import { isUNC, randomPath } from '../../base/common/extpath.ts';
import { Utils } from '../../platform/profiling/common/profiling.ts';
...
export async function main(argv: string[]): Promise<void> {
	...
	// Extensions Management
		...
		if (process.env['VSCODE_DEV']) {
			cliProcessMain = './cliProcessMain.ts';
		} else {
			cliProcessMain = './vs/code/node/cliProcessMain.ts';
		}
