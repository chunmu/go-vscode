...
import { IRemoteConsoleLog } from '../../../base/common/console.ts';
import { SerializedError } from '../../../base/common/errors.ts';
...
import { IAuthorizationProtectedResourceMetadata, IAuthorizationServerMetadata, IAuthorizationTokenResponse } from '../../../base/common/oauth.ts';
import * as performance from '../../../base/common/performance.ts';
import Severity from '../../../base/common/severity.ts';
import { ThemeColor, ThemeIcon } from '../../../base/common/themables.ts';
import { URI, UriComponents, UriDto } from '../../../base/common/uri.ts';
import { RenderLineNumbersType, TextEditorCursorStyle } from '../../../editor/common/config/editorOptions.ts';
...
import { CompletionItemLabel } from '../../../editor/common/languages.ts';
import { CharacterPair, CommentRule, EnterAction } from '../../../editor/common/languages/languageConfiguration.ts';
import { EndOfLineSequence } from '../../../editor/common/model.ts';
...
import { ConfigurationScope } from '../../../platform/configuration/common/configurationRegistry.ts';
import { IExtensionIdWithVersion } from '../../../platform/extensionManagement/common/extensionStorage.ts';
...
import { ResourceLabelFormatter } from '../../../platform/label/common/label.ts';
import { ILoggerOptions, ILoggerResource, LogLevel } from '../../../platform/log/common/log.ts';
import { IMarkerData } from '../../../platform/markers/common/markers.ts';
import { IProgressOptions, IProgressStep } from '../../../platform/progress/common/progress.ts';
import * as quickInput from '../../../platform/quickinput/common/quickInput.ts';
import { IRemoteConnectionData, TunnelDescription } from '../../../platform/remote/common/remoteAuthorityResolver.ts';
import { AuthInfo, Credentials } from '../../../platform/request/common/request.ts';
import { ClassifiedEvent, IGDPRProperty, OmitMetadata, StrictPropertyCheck } from '../../../platform/telemetry/common/gdprTypings.ts';
import { TelemetryLevel } from '../../../platform/telemetry/common/telemetry.ts';
import { ISerializableEnvironmentDescriptionMap, ISerializableEnvironmentVariableCollection } from '../../../platform/terminal/common/environmentVariable.ts';
import { ICreateContributedTerminalProfileOptions, IProcessProperty, IProcessReadyWindowsPty, IShellLaunchConfigDto, ITerminalEnvironment, ITerminalLaunchError, ITerminalProfile, TerminalExitReason, TerminalLocation, TerminalShellType } from '../../../platform/terminal/common/terminal.ts';
import { ProvidedPortAttributes, TunnelCreationOptions, TunnelOptions, TunnelPrivacyId, TunnelProviderFeatures } from '../../../platform/tunnel/common/tunnel.ts';
import { EditSessionIdentityMatch } from '../../../platform/workspace/common/editSessions.ts';
import { WorkspaceTrustRequestOptions } from '../../../platform/workspace/common/workspaceTrust.ts';
import { SaveReason } from '../../common/editor.ts';
import { IRevealOptions, ITreeItem, IViewBadge } from '../../common/views.ts';
import { CallHierarchyItem } from '../../contrib/callHierarchy/common/callHierarchy.ts';
import { IChatAgentMetadata, IChatAgentRequest, IChatAgentResult, UserSelectedTools } from '../../contrib/chat/common/chatAgents.ts';
import { ICodeMapperRequest, ICodeMapperResult } from '../../contrib/chat/common/chatCodeMapperService.ts';
import { IChatContextItem, IChatContextSupport } from '../../contrib/chat/common/chatContext.ts';
import { IChatRelatedFile, IChatRelatedFileProviderMetadata as IChatRelatedFilesProviderMetadata, IChatRequestDraft } from '../../contrib/chat/common/chatEditingService.ts';
import { IChatProgressHistoryResponseContent, IChatRequestVariableData } from '../../contrib/chat/common/chatModel.ts';
import { ChatResponseClearToPreviousToolInvocationReason, IChatContentInlineReference, IChatExternalEditsDto, IChatFollowup, IChatNotebookEdit, IChatProgress, IChatTask, IChatTaskDto, IChatUserActionEvent, IChatVoteAction } from '../../contrib/chat/common/chatService.ts';
import { IChatSessionItem, IChatSessionProviderOptionGroup, IChatSessionProviderOptionItem } from '../../contrib/chat/common/chatSessionsService.ts';
import { IChatRequestVariableValue } from '../../contrib/chat/common/chatVariables.ts';
import { ChatAgentLocation } from '../../contrib/chat/common/constants.ts';
import { IChatMessage, IChatResponsePart, ILanguageModelChatMetadataAndIdentifier, ILanguageModelChatSelector } from '../../contrib/chat/common/languageModels.ts';
import { IPreparedToolInvocation, IToolInvocation, IToolInvocationPreparationContext, IToolProgressStep, IToolResult, ToolDataSource } from '../../contrib/chat/common/languageModelToolsService.ts';
import { DebugConfigurationProviderTriggerKind, IAdapterDescriptor, IConfig, IDebugSessionReplMode, IDebugTestRunReference, IDebugVisualization, IDebugVisualizationContext, IDebugVisualizationTreeItem, MainThreadDebugVisualization } from '../../contrib/debug/common/debug.ts';
import { McpCollectionDefinition, McpConnectionState, McpServerDefinition, McpServerLaunch } from '../../contrib/mcp/common/mcpTypes.ts';
...
import { ICellExecutionComplete, ICellExecutionStateUpdate } from '../../contrib/notebook/common/notebookExecutionStateService.ts';
import { ICellRange } from '../../contrib/notebook/common/notebookRange.ts';
import { ISCMHistoryOptions } from '../../contrib/scm/common/history.ts';
import { InputValidationType } from '../../contrib/scm/common/scm.ts';
import { IWorkspaceSymbol, NotebookPriorityInfo } from '../../contrib/search/common/search.ts';
import { IRawClosedNotebookFileMatch } from '../../contrib/search/common/searchNotebookHelpers.ts';
import { IKeywordRecognitionEvent, ISpeechProviderMetadata, ISpeechToTextEvent, ITextToSpeechEvent } from '../../contrib/speech/common/speechService.ts';
import { CoverageDetails, ExtensionRunTestsRequest, ICallProfileRunHandler, IFileCoverage, ISerializedTestResults, IStartControllerTests, ITestItem, ITestMessage, ITestRunProfile, ITestRunTask, ResolvedTestRunRequest, TestControllerCapability, TestMessageFollowupRequest, TestMessageFollowupResponse, TestResultState, TestsDiffOp } from '../../contrib/testing/common/testTypes.ts';
import { Timeline, TimelineChangeEvent, TimelineOptions, TimelineProviderDescriptor } from '../../contrib/timeline/common/timeline.ts';
import { TypeHierarchyItem } from '../../contrib/typeHierarchy/common/typeHierarchy.ts';
import { RelatedInformationResult, RelatedInformationType } from '../../services/aiRelatedInformation/common/aiRelatedInformation.ts';
import { AiSettingsSearchProviderOptions, AiSettingsSearchResult } from '../../services/aiSettingsSearch/common/aiSettingsSearch.ts';
import { AuthenticationSession, AuthenticationSessionAccount, AuthenticationSessionsChangeEvent, IAuthenticationConstraint, IAuthenticationCreateSessionOptions, IAuthenticationGetSessionsOptions, IAuthenticationWwwAuthenticateRequest } from '../../services/authentication/common/authentication.ts';
import { EditorGroupColumn } from '../../services/editor/common/editorGroupColumn.ts';
import { IExtensionDescriptionDelta, IStaticWorkspaceData } from '../../services/extensions/common/extensionHostProtocol.ts';
import { IResolveAuthorityResult } from '../../services/extensions/common/extensionHostProxy.ts';
import { ActivationKind, ExtensionActivationReason, MissingExtensionDependency } from '../../services/extensions/common/extensions.ts';
import { Dto, IRPCProtocol, SerializableObjectWithBuffers, createProxyIdentifier } from '../../services/extensions/common/proxyIdentifier.ts';
import { IInlineCompletionsUnificationState } from '../../services/inlineCompletions/common/inlineCompletionsUnification.ts';
import { ILanguageStatus } from '../../services/languageStatus/common/languageStatusService.ts';
import { OutputChannelUpdateMode } from '../../services/output/common/output.ts';
import { CandidatePort } from '../../services/remote/common/tunnelModel.ts';
import { IFileQueryBuilderOptions, ITextQueryBuilderOptions } from '../../services/search/common/queryBuilder.ts';
import * as search from '../../services/search/common/search.ts';
import { AISearchKeyword, TextSearchCompleteMessage } from '../../services/search/common/searchExtTypes.ts';
import { ISaveProfileResult } from '../../services/userDataProfile/common/userDataProfile.ts';
import { IExtHostDocumentSaveDelegate } from './extHostDocumentData.ts';
