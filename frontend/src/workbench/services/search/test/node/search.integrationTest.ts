...
import { IFolderQuery, QueryType, IRawFileMatch } from '../../common/search.ts';
import { Engine as FileSearchEngine, FileWalker } from '../../node/fileSearch.ts';
...
flakySuite('FileWalker', () => {
	...
	(platform.isWindows ? test.skip : test)('Find: exclude folder path suffix', function (done: () => void) {
		const file0 = './examples/company.ts';
		const file1 = './examples/subfolder/subfile.txt';
