...
import { isMacintosh, isWeb } from '../../../../base/common/platform.ts';
...
import { ICommandActionTitle, Icon } from '../../../../platform/action/common/action.ts';
...
import { IWorkbenchContributionsRegistry, Extensions as WorkbenchExtensions, WorkbenchPhase, registerWorkbenchContribution2 } from '../../../common/contributions.ts';
import { EditorExtensions } from '../../../common/editor.ts';
import { IViewContainersRegistry, IViewsRegistry, ViewContainer, ViewContainerLocation, Extensions as ViewExtensions } from '../../../common/views.ts';
...
import { COPY_NOTEBOOK_VARIABLE_VALUE_ID, COPY_NOTEBOOK_VARIABLE_VALUE_LABEL } from '../../notebook/browser/contrib/notebookVariables/notebookVariableCommands.ts';
...
import { DebugWatchAccessibilityAnnouncer } from '../common/debugAccessibilityAnnouncer.ts';
import { DebugContentProvider } from '../common/debugContentProvider.ts';
import { DebugLifecycle } from '../common/debugLifecycle.ts';
import { DebugVisualizerService, IDebugVisualizerService } from '../common/debugVisualizers.ts';
import { DisassemblyViewInput } from '../common/disassemblyViewInput.ts';
import { ReplAccessibilityAnnouncer } from '../common/replAccessibilityAnnouncer.ts';
import { BreakpointEditorContribution } from './breakpointEditorContribution.ts';
import { BreakpointsView } from './breakpointsView.ts';
import { CallStackEditorContribution } from './callStackEditorContribution.ts';
import { CallStackView } from './callStackView.ts';
import { registerColors } from './debugColors.ts';
import { ADD_CONFIGURATION_ID, ADD_TO_WATCH_ID, ADD_TO_WATCH_LABEL, CALLSTACK_BOTTOM_ID, CALLSTACK_BOTTOM_LABEL, CALLSTACK_DOWN_ID, CALLSTACK_DOWN_LABEL, CALLSTACK_TOP_ID, CALLSTACK_TOP_LABEL, CALLSTACK_UP_ID, CALLSTACK_UP_LABEL, CONTINUE_ID, CONTINUE_LABEL, COPY_EVALUATE_PATH_ID, COPY_EVALUATE_PATH_LABEL, COPY_STACK_TRACE_ID, COPY_VALUE_ID, COPY_VALUE_LABEL, DEBUG_COMMAND_CATEGORY, DEBUG_CONSOLE_QUICK_ACCESS_PREFIX, DEBUG_QUICK_ACCESS_PREFIX, DEBUG_RUN_COMMAND_ID, DEBUG_RUN_LABEL, DEBUG_START_COMMAND_ID, DEBUG_START_LABEL, DISCONNECT_AND_SUSPEND_ID, DISCONNECT_AND_SUSPEND_LABEL, DISCONNECT_ID, DISCONNECT_LABEL, EDIT_EXPRESSION_COMMAND_ID, JUMP_TO_CURSOR_ID, NEXT_DEBUG_CONSOLE_ID, NEXT_DEBUG_CONSOLE_LABEL, OPEN_LOADED_SCRIPTS_LABEL, PAUSE_ID, PAUSE_LABEL, PREV_DEBUG_CONSOLE_ID, PREV_DEBUG_CONSOLE_LABEL, REMOVE_EXPRESSION_COMMAND_ID, RESTART_FRAME_ID, RESTART_LABEL, RESTART_SESSION_ID, SELECT_AND_START_ID, SELECT_AND_START_LABEL, SELECT_DEBUG_CONSOLE_ID, SELECT_DEBUG_CONSOLE_LABEL, SELECT_DEBUG_SESSION_ID, SELECT_DEBUG_SESSION_LABEL, SET_EXPRESSION_COMMAND_ID, SHOW_LOADED_SCRIPTS_ID, STEP_INTO_ID, STEP_INTO_LABEL, STEP_INTO_TARGET_ID, STEP_INTO_TARGET_LABEL, STEP_OUT_ID, STEP_OUT_LABEL, STEP_OVER_ID, STEP_OVER_LABEL, STOP_ID, STOP_LABEL, TERMINATE_THREAD_ID, TOGGLE_INLINE_BREAKPOINT_ID, COPY_ADDRESS_ID, COPY_ADDRESS_LABEL, TOGGLE_BREAKPOINT_ID, BREAK_WHEN_VALUE_CHANGES_ID, BREAK_WHEN_VALUE_IS_ACCESSED_ID, BREAK_WHEN_VALUE_IS_READ_ID } from './debugCommands.ts';
import { DebugConsoleQuickAccess } from './debugConsoleQuickAccess.ts';
import { RunToCursorAction, SelectionToReplAction, SelectionToWatchExpressionsAction } from './debugEditorActions.ts';
import { DebugEditorContribution } from './debugEditorContribution.ts';
import * as icons from './debugIcons.ts';
import { DebugProgressContribution } from './debugProgress.ts';
import { StartDebugQuickAccessProvider } from './debugQuickAccess.ts';
import { DebugService } from './debugService.ts';
import './debugSettingMigration.ts';
import { DebugStatusContribution } from './debugStatus.ts';
import { DebugTitleContribution } from './debugTitle.ts';
import { DebugToolBar } from './debugToolBar.ts';
import { DebugViewPaneContainer } from './debugViewlet.ts';
import { DisassemblyView, DisassemblyViewContribution } from './disassemblyView.ts';
import { LoadedScriptsView } from './loadedScriptsView.ts';
import './media/debug.contribution.css';
import './media/debugHover.css';
import { Repl } from './repl.ts';
import { ReplAccessibilityHelp } from './replAccessibilityHelp.ts';
import { ReplAccessibleView } from './replAccessibleView.ts';
import { RunAndDebugAccessibilityHelp } from './runAndDebugAccessibilityHelp.ts';
import { StatusBarColorProvider } from './statusbarColorProvider.ts';
import { SET_VARIABLE_ID, VIEW_MEMORY_ID, VariablesView } from './variablesView.ts';
import { ADD_WATCH_ID, ADD_WATCH_LABEL, REMOVE_WATCH_EXPRESSIONS_COMMAND_ID, REMOVE_WATCH_EXPRESSIONS_LABEL, WatchExpressionsView } from './watchExpressionsView.ts';
import { WelcomeView } from './welcomeView.ts';
