...
import { MarkdownString, isMarkdownString } from '../../../../base/common/htmlContent.ts';
...
import { Extensions as ConfigurationExtensions, ConfigurationScope, IConfigurationNode, IConfigurationRegistry } from '../../../../platform/configuration/common/configurationRegistry.ts';
import { SyncDescriptor } from '../../../../platform/instantiation/common/descriptors.ts';
import { InstantiationType, registerSingleton } from '../../../../platform/instantiation/common/extensions.ts';
import { IInstantiationService } from '../../../../platform/instantiation/common/instantiation.ts';
import { McpAccessValue, McpAutoStartValue, mcpAccessConfig, mcpAutoStartConfig, mcpGalleryServiceEnablementConfig, mcpGalleryServiceUrlConfig } from '../../../../platform/mcp/common/mcpManagement.ts';
...
import { AddConfigurationType, AssistedTypes } from '../../mcp/browser/mcpCommandsAddConfiguration.ts';
import { allDiscoverySources, discoverySourceSettingsLabel, mcpDiscoverySection, mcpServerSamplingSection } from '../../mcp/common/mcpConfiguration.ts';
import { ChatAgentNameService, ChatAgentService, IChatAgentNameService, IChatAgentService } from '../common/chatAgents.ts';
import { CodeMapperService, ICodeMapperService } from '../common/chatCodeMapperService.ts';
import '../common/chatColors.ts';
import { IChatEditingService } from '../common/chatEditingService.ts';
import { IChatLayoutService } from '../common/chatLayoutService.ts';
import { ChatModeService, IChatMode, IChatModeService } from '../common/chatModes.ts';
import { ChatResponseResourceFileSystemProvider } from '../common/chatResponseResourceFileSystemProvider.ts';
...
import { ChatSlashCommandService, IChatSlashCommandService } from '../common/chatSlashCommands.ts';
import { ChatTodoListService, IChatTodoListService } from '../common/chatTodoListService.ts';
...
import { ChatWidgetHistoryService, IChatWidgetHistoryService } from '../common/chatWidgetHistoryService.ts';
import { ChatAgentLocation, ChatConfiguration, ChatModeKind } from '../common/constants.ts';
import { ILanguageModelIgnoredFilesService, LanguageModelIgnoredFilesService } from '../common/ignoredFiles.ts';
import { ILanguageModelsService, LanguageModelsService } from '../common/languageModels.ts';
import { ILanguageModelStatsService, LanguageModelStatsService } from '../common/languageModelStats.ts';
...
import { ChatPromptFilesExtensionPointHandler } from '../common/promptSyntax/chatPromptFilesContribution.ts';
import { PromptsConfig } from '../common/promptSyntax/config/config.ts';
import { INSTRUCTIONS_DEFAULT_SOURCE_FOLDER, INSTRUCTION_FILE_EXTENSION, LEGACY_MODE_DEFAULT_SOURCE_FOLDER, LEGACY_MODE_FILE_EXTENSION, PROMPT_DEFAULT_SOURCE_FOLDER, PROMPT_FILE_EXTENSION } from '../common/promptSyntax/config/promptFileLocations.ts';
import { PromptLanguageFeaturesProvider } from '../common/promptSyntax/promptFileContributions.ts';
import { AGENT_DOCUMENTATION_URL, INSTRUCTIONS_DOCUMENTATION_URL, PROMPT_DOCUMENTATION_URL } from '../common/promptSyntax/promptTypes.ts';
import { IPromptsService } from '../common/promptSyntax/service/promptsService.ts';
import { PromptsService } from '../common/promptSyntax/service/promptsServiceImpl.ts';
import { LanguageModelToolsExtensionPointHandler } from '../common/tools/languageModelToolsContribution.ts';
import { BuiltinToolsContribution } from '../common/tools/tools.ts';
import { IVoiceChatService, VoiceChatService } from '../common/voiceChatService.ts';
import { registerChatAccessibilityActions } from './actions/chatAccessibilityActions.ts';
import { AgentChatAccessibilityHelp, EditsChatAccessibilityHelp, PanelChatAccessibilityHelp, QuickChatAccessibilityHelp } from './actions/chatAccessibilityHelp.ts';
import { ACTION_ID_NEW_CHAT, CopilotTitleBarMenuRendering, ModeOpenChatGlobalAction, registerChatActions } from './actions/chatActions.ts';
import { CodeBlockActionRendering, registerChatCodeBlockActions, registerChatCodeCompareBlockActions } from './actions/chatCodeblockActions.ts';
import { ChatContextContributions } from './actions/chatContext.ts';
import { registerChatContextActions } from './actions/chatContextActions.ts';
import { ContinueChatInSessionActionRendering } from './actions/chatContinueInAction.ts';
import { registerChatCopyActions } from './actions/chatCopyActions.ts';
import { registerChatDeveloperActions } from './actions/chatDeveloperActions.ts';
import { ChatSubmitAction, registerChatExecuteActions } from './actions/chatExecuteActions.ts';
import { registerChatFileTreeActions } from './actions/chatFileTreeActions.ts';
import { ChatGettingStartedContribution } from './actions/chatGettingStarted.ts';
import { registerChatExportActions } from './actions/chatImportExport.ts';
import { registerLanguageModelActions } from './actions/chatLanguageModelActions.ts';
import { registerMoveActions } from './actions/chatMoveActions.ts';
import { registerNewChatActions } from './actions/chatNewActions.ts';
import { registerChatPromptNavigationActions } from './actions/chatPromptNavigationActions.ts';
import { registerQuickChatActions } from './actions/chatQuickInputActions.ts';
...
import { registerChatTitleActions } from './actions/chatTitleActions.ts';
import { registerChatElicitationActions } from './actions/chatElicitationActions.ts';
import { registerChatToolActions } from './actions/chatToolActions.ts';
import { ChatTransferContribution } from './actions/chatTransfer.ts';
import './agentSessions/agentSessionsView.ts';
import { IChatAccessibilityService, IChatCodeBlockContextProviderService, IChatWidgetService, IQuickChatService } from './chat.ts';
import { ChatAccessibilityService } from './chatAccessibilityService.ts';
import './chatAttachmentModel.ts';
import { ChatAttachmentResolveService, IChatAttachmentResolveService } from './chatAttachmentResolveService.ts';
import { ChatMarkdownAnchorService, IChatMarkdownAnchorService } from './chatContentParts/chatMarkdownAnchorService.ts';
import { ChatContextPickService, IChatContextPickService } from './chatContextPickService.ts';
import { ChatInputBoxContentProvider } from './chatEdinputInputContentProvider.ts';
import { ChatEditingEditorAccessibility } from './chatEditing/chatEditingEditorAccessibility.ts';
import { registerChatEditorActions } from './chatEditing/chatEditingEditorActions.ts';
import { ChatEditingEditorContextKeys } from './chatEditing/chatEditingEditorContextKeys.ts';
import { ChatEditingEditorOverlay } from './chatEditing/chatEditingEditorOverlay.ts';
import { ChatEditingService } from './chatEditing/chatEditingServiceImpl.ts';
import { ChatEditingNotebookFileSystemProviderContrib } from './chatEditing/notebook/chatEditingNotebookFileSystemProvider.ts';
import { SimpleBrowserOverlay } from './chatEditing/simpleBrowserEditorOverlay.ts';
import { ChatEditor, IChatEditorOptions } from './chatEditor.ts';
import { ChatEditorInput, ChatEditorInputSerializer } from './chatEditorInput.ts';
import { ChatLayoutService } from './chatLayoutService.ts';
import './chatManagement/chatManagement.contribution.ts';
import { agentSlashCommandToMarkdown, agentToMarkdown } from './chatMarkdownDecorationsRenderer.ts';
import { ChatOutputRendererService, IChatOutputRendererService } from './chatOutputItemRenderer.ts';
import { ChatCompatibilityNotifier, ChatExtensionPointHandler } from './chatParticipant.contribution.ts';
import { ChatPasteProvidersFeature } from './chatPasteProviders.ts';
import { QuickChatService } from './chatQuick.ts';
import { ChatResponseAccessibleView } from './chatResponseAccessibleView.ts';
import { ChatTerminalOutputAccessibleView } from './chatTerminalOutputAccessibleView.ts';
import { LocalChatSessionsProvider } from './chatSessions/localChatSessionsProvider.ts';
import { ChatSessionsView, ChatSessionsViewContrib } from './chatSessions/view/chatSessionsView.ts';
import { ChatSetupContribution, ChatTeardownContribution } from './chatSetup.ts';
import { ChatStatusBarEntry } from './chatStatus.ts';
import { ChatVariablesService } from './chatVariables.ts';
import { ChatWidget } from './chatWidget.ts';
import { ChatCodeBlockContextProviderService } from './codeBlockContextProviderService.ts';
import { ChatDynamicVariableModel } from './contrib/chatDynamicVariables.ts';
import { ChatImplicitContextContribution } from './contrib/chatImplicitContext.ts';
import './contrib/chatInputCompletions.ts';
import './contrib/chatInputEditorContrib.ts';
import './contrib/chatInputEditorHover.ts';
import { ChatRelatedFilesContribution } from './contrib/chatInputRelatedFilesContrib.ts';
import { LanguageModelToolsConfirmationService } from './languageModelToolsConfirmationService.ts';
import { LanguageModelToolsService, globalAutoApproveDescription } from './languageModelToolsService.ts';
import './promptSyntax/promptCodingAgentActionContribution.ts';
import './promptSyntax/promptToolsCodeLensProvider.ts';
import { PromptUrlHandler } from './promptSyntax/promptUrlHandler.ts';
import { ConfigureToolSets, UserToolSetsContributions } from './tools/toolSetsContribution.ts';
import { ChatViewsWelcomeHandler } from './viewsWelcome/chatViewsWelcomeHandler.ts';
import { ChatWidgetService } from './chatWidgetService.ts';
